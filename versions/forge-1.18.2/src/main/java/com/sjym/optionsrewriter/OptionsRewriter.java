...
public final class OptionsRewriter {
    ...
    public OptionsRewriter() {
        ModLoadingContext.get().registerExtensionPoint(ConfigGuiHandler.ConfigGuiFactory.class,
                () -> new ConfigGuiHandler.ConfigGuiFactory((minecraft, parent) -> new OptionsRewriterConfigScreen(parent)));
    }

    @Mod.EventBusSubscriber(modid = MODID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
    public static final class ClientEvents {
        @SubscribeEvent
        public static void onTitleScreen(ScreenEvent.InitScreenEvent.Post event) {
        ...
        public static void onClientTick(TickEvent.ClientTickEvent event) {
            Minecraft minecraft = Minecraft.getInstance();
            if (event.phase == TickEvent.Phase.END && pendingNotice && minecraft.screen instanceof TitleScreen) {
                pendingNotice = false;
                minecraft.setScreen(new OptionsRewriterNoticeScreen(minecraft.screen, settings.restartMessage()));
            }
        }
    }
}
